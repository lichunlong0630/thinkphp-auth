...
trait Jump
{
    ...
    protected function info($msg = '', string $url = null, $data = '')
    {
        $result = [
            'msg' => $msg,
            'url' => $url
        ];
        $response = Response::create($this->app->config->get('jump.dispatch_info_tmpl'), 'view')->assign($result);
        throw new HttpResponseException($response);
    }
